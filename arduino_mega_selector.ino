...
void loop() {
  if (Serial.available() > 0) { // เช็คว่ามีข้อมูลจาก Serial หรือไม่
    ...
    if (python == 'a') {
      Serial.println("Evangelion - Cruel Angel Thesis");
      Evangelion(); // เรียกฟังก์ชันเล่นเพลง
      
    }
    else if (python == 'b') {
      Serial.println("Conan OST");
      conan();
      
    }
    else if (python == 'c') {
      Serial.println("Night Dancer");
      NightDancer();
      
    }
    else if (python == 'd') {
      Serial.println("Chihatan");
      Chihatan();
      
    }
    else if (python == 'e') {
      Serial.println("Racing Into The Night");
...
void conan(){
  ...
  digitalWrite(led_red, HIGH);
  tone(buzzer_Melody, 0);
  Serial.println("Stop"); //ส่งค่ากลับ
}
////////////////////////////////////////////////////////////////////////////
